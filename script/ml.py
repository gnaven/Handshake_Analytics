...
def ml(dfin, models=None, y_col=None, weight_plot=0):
    ...
    for model in models:
        ...
        for i, (train, test) in enumerate(skf.split(X_nd, y_n)):
            ...
            train_n = len(train)
            # empirically found that dev 1/4 is good
            dev = train[:int(train_n/4)]
            ...
            for c in c_l:
                if model == 'LOGISTIC':
                    clf = linear_model.LogisticRegression(
                        penalty=regularizer, C=c)
                elif model == 'KNN':
                    clf = KNeighborsClassifier(
                        n_neighbors=c, metric='euclidean', weights='uniform')
                else:
            ...
            print('fold:', i, ' best c:', best_c, ' dev:%.2f' % best_acc,
                  ' dev_ones:%.2f' % (y_n[dev].sum()/len(dev)), end='')
            if model == 'LOGISTIC':
                clf = linear_model.LogisticRegression(
                    penalty=regularizer, C=best_c)
                clf1 = linear_model.LogisticRegression(penalty=regularizer,
                                                       C=best_c, n_jobs=-1)
            elif model == 'KNN':
                clf = KNeighborsClassifier(
                    n_neighbors=best_c, metric='euclidean', weights='uniform')
            else:
...
def plot(f0, f1, df):
    ...
    plt.scatter(df[f0], df[f1], c=[
                'g' if x else 'r' for x in df['y']], alpha=0.3)
    plt.xlabel(f0)
    plt.ylabel(f1)
    plt.show()
...
def run_models(df):
    ...
    for y in y_list:
        ...
        feat_names = ['US Citizen', 'School Year Name', 'Educations Cumulative Gpa', 'Engaged_Fair',
